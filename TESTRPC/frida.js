...
// moudlule name, exportname으로 찾는 헬퍼 함수 추가(frida17.x 부터  Module.findExportByName API 삭제)
function tryFindExportByName(moduleName, exportName) {
    const m = Process.findModuleByName(moduleName);
    if (m === null) return null;

    const exps = m.enumerateExports();
    for (let i = 0; i < exps.length; i++) {
        if (exps[i].name === exportName) {
            return exps[i].address; // NativePointer
        }
    }
    return null;
}
...
rpc.exports = {
    setupOepTracing: function (moduleName, expectedOepRanges) {
        ...
        const loadDll = tryFindExportByName('ntdll', 'LdrLoadDll');
        ...
        const ntProtectVirtualMemory = tryFindExportByName('ntdll', 'NtProtectVirtualMemory');
        ...
        const activateActivationContext = tryFindExportByName('ntdll', 'RtlActivateActivationContextUnsafeFast');
        const activateActivationContextListener = Interceptor.attach(activateActivationContext, {
            onLeave: function (_args) {
                ...
                const initializeFusion = tryFindExportByName('clr.dll', 'InitializeFusion');
    ...
    enumerateExportedFunctions: function (excludedModuleName) {
        ...
        log("enumerateExportedFunctions return.....");
        return exports;
    },
